...
double countFreeVolume(const octomap::OcTree *octree)
{
    ...
    {
        if (!octree->isNodeOccupied(*n))//未被占据
            volume += pow(n.getSize(), 3);
    }
    return volume;
}

// Input: octree, position, direction. Output: hits 输入：八叉树 位置， 方向    输出 碰撞
...
vector<vector<point3d>> extractFrontierPoints(const octomap::OcTree *octree)
{
    ...
    double distance;
    double R1 = 0.4; //group length 分组的距离阈值 
    ...
    {
        frontier_true = false;
        unsigned long int num_free = 0; //number of free cube around frontier, for filtering out fake frontier自由立方体数量 过滤伪边界
...
//生成移动候选者。输入sensor_orig和initial_yaw，输出候选
//senor_orig：传感器的位置。 initial_yaw：传感器的偏航方向
vector<pair<point3d, point3d>> extractCandidateViewPoints(vector<vector<point3d>> frontier_groups, point3d sensor_orig)
{
    //   cout << "*** extract Candidate View Points running; frontier_groups size = " << frontier_groups.size()
    ...
    for (vector<vector<point3d>>::size_type u = 0; u < frontier_groups.size(); u++)
    {
        for (double yaw = 0; yaw < 2 * PI; yaw += PI * 2 / n)
        {
            ...
            {
                // check candidate to other frontiers; 到达其他候选点的距离不能太近
                ...
                for (double x_buf = x - 0.3; x_buf < x + 0.3; x_buf += octo_reso)
                    for (double y_buf = y - 0.3; y_buf < y + 0.3; y_buf += octo_reso)
                        for (double z_buf = sensor_orig.z() - 0.2; z_buf < sensor_orig.z() + 0.2; z_buf += octo_reso)
                        {
                            ...
                            else if (cur_tree->isNodeOccupied(n_cur_3d))//此处为障碍物 
                            {
            ...
            if (candidate_valid)
            {
                candidates.push_back(make_pair<point3d, point3d>(point3d(x, y, z), point3d(0.0, 0.0, yaw)));//返回目标位置和航向角
            }
        }
    }
    return candidates;
...
double calculateMutualInformation(const octomap::OcTree *octree, const point3d &sensor_orig, const octomap::Pointcloud &hits, const double before)
{
    ...
    double after = countFreeVolume(octree_copy);//计算空的立方体的体积
    delete octree_copy;//删除占据位置
    return after - before;//返回互信息
}
...
void kinectCallbacks(const sensor_msgs::PointCloud2ConstPtr &cloud2_msg)
{
    ...
    while (!pcl_ros::transformPointCloud("/map", *cloud_local, *cloud, *tf_listener))//返回坐标变换
    {
...
void laserscanCallbacks(const sensor_msgs::PointCloud2ConstPtr &cloud2_msg)
{
    ...
    cur_tree_2d->insertPointCloud(hits, laser_orig, 5.6);//更新2d地图的结构
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        //frontier_groups.clear();//in the next line 第一步：计算扫描的线
	...
	*/
        ...
        while (candidates.size() < 1)
        {
            ...
            // Rotate another 60 degrees 没有找到候选点，继续旋转60度
            twist_cmd.linear.x = twist_cmd.linear.y = twist_cmd.angular.z = 0;
            ros::Time start_turn = ros::Time::now();
            ...
            while (ros::Time::now() - start_turn < ros::Duration(2.6))
            {                              // turning duration - second
                twist_cmd.angular.z = 0.4; // turning speed 花2.6秒时间转60度
                pub_twist.publish(twist_cmd);
                ros::Duration(0.05).sleep();
            }
            // stop
...
#pragma omp parallel for
    ...
    loop:
        ...
        // Publish the goal as a Marker in rviz 在rviz发布目标点
        ...
        // Send the Robot
        Goal_heading.setRPY(0.0, 0.0, candidates[max_order[p]].second.yaw()); //设置机器人的航向角

        arrived = goToDest(next_vp, Goal_heading); //控制机器人到达目标状态
        ...
        if (arrived)
        {
            ...
            while (!got_tf)
            {
                try
                {
                    //tf_listener->lookupTransform("/map", "/camera_rgb_frame", ros::Time(0), transform);// need to change tf of kinect###############
                    tf_listener->lookupTransform("/map", "kinect2_rgb_optical_frame", ros::Time(0), transform); // need to change tf of kinect###############
                    velo_orig = point3d(transform.getOrigin().x(), transform.getOrigin().y(), transform.getOrigin().z()); // 获取定位的状态
                    got_tf = true;
                }
                catch (tf::TransformException ex)
                {
            ...
            for (octomap::OcTree::leaf_iterator n = cur_tree->begin_leafs(cur_tree_2d->getTreeDepth()); n != cur_tree->end_leafs(); ++n)
            { // changed there#######
                if (cur_tree->isNodeOccupied(*n))
                    continue;
                p.x = n.getX();
